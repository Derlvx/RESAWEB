document.addEventListener("DOMContentLoaded", function () {
    ...
    var popupLondon = L.popup([51.50, -0.12], { content: '<img src="IMG/popupStore2.jpeg" alt=""><h3>Horizon London</h3><p>235 Regent Street, London W1B 2EL</p><div class="selectButton"><input class="radio" type="radio" name="location" id="radio2" value="London" onclick="selectMap()" required><label for="radio2">select</label><div class="border full-rounded"></div></div>', className: 'popupText' });
    markerLondon.bindPopup(popupLondon);

    var markerBerlin = L.marker([52.51, 13.38], { alt: 'Berlin' }).addTo(map);
    var popupBerlin = L.popup([52.51, 13.38], { content: '<img src="IMG/popupStore3.jpeg" alt=""><h3>Horizon Berlin</h3><p>Kurfürstendamm 26, 10719 Berlin</p><div class="selectButton"><input class="radio" type="radio" name="location" id="radio3" value="Berlin" onclick="selectMap()" required><label for="radio3">select</label><div class="border full-rounded"></div></div>', className: 'popupText' });
    markerBerlin.bindPopup(popupBerlin);

    var markerMadrid = L.marker([40.41, -3.70], { alt: 'Madrid' }).addTo(map);
    var popupMadrid = L.popup([40.41, -3.70], { content: '<img src="IMG/popupStore4.jpeg" alt=""><h3>Horizon Madrid</h3><p>Prta del Sol, 1, 28013 Madrid</p><div class="selectButton"><input class="radio" type="radio" name="location" id="radio4" value="Madrid" onclick="selectMap()" required><label for="radio4">select</label><div class="border full-rounded"></div></div>', className: 'popupText' });
    markerMadrid.bindPopup(popupMadrid);

    var markerBruxelles = L.marker([50.84, 4.35], { alt: 'Bruxelles' }).addTo(map);
    var popupBruxelles = L.popup([50.84, 4.35], { content: '<img src="IMG/popupStore5.jpeg" alt=""><h3>Horizon Bruxelles</h3><p>Av. de la Toison Or 2628, 1050 Bruxelles</p><div class="selectButton"><input class="radio" type="radio" name="location" id="radio5" value="Bruxelles" onclick="selectMap()" required><label for="radio5">select</label><div class="border full-rounded"></div></div>', className: 'popupText' });
    markerBruxelles.bindPopup(popupBruxelles);

    var markerBerne = L.marker([46.94, 7.45], { alt: 'Berne' }).addTo(map);
    var popupBerne = L.popup([46.94, 7.45], { content: '<img src="IMG/popupStore6.jpeg" alt=""><h3>Horizon Berne</h3><p>Marktgasse 60, 3011 Bern</p><div class="selectButton"><input class="radio" type="radio" name="location" id="radio6" value="Berne" onclick="selectMap()" required><label for="radio6">select</label><div class="border full-rounded"></div></div>', className: 'popupText' });
